...
def index():
    return '<?xml version="1.0"?><response><status>'+ ('success' if random.randint(1,20) > 5 else 'failed') +'</status><price>' + str(random.randint(1,20)*random.randint(0,100)/100) + '</price></response>'

@hug.get('/json')
def index():
    return { 'status': ('success' if random.randint(1,20) > 5 else 'failed'), 'price': random.randint(1,20)*random.randint(0,100)/100 }
...
def token_gen_call(password, username=None, \
                    email = None):
